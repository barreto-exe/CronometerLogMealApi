...
public class CronometerPollingHostedService : BackgroundService
{
    ...
    private async Task<ImageOcrResult> ProcessImageAsync(byte[] imageBytes, string? caption, CancellationToken ct)
    {
        ...
        var prompt = GeminiPrompts.ImageOcrPrompt.Replace("@Now", venezuelaNow.ToString("yyyy-MM-ddTHH:mm:ss"));
        
        if (!string.IsNullOrWhiteSpace(caption))
        {
            prompt += $"\n\nINSTRUCCIONES ADICIONALES DEL USUARIO: {caption}";
        }

        // Use Gemini Vision API for image analysis
        var response = await _geminiClient.AnalyzeImageAsync(prompt, imageBytes, "image/jpeg", ct);
        var content = response?.Candidates?.FirstOrDefault()?.Content?.Parts?.FirstOrDefault()?.Text;

        if (string.IsNullOrEmpty(content))
        {
            return new ImageOcrResult { Error = "No se pudo analizar la imagen." };
        }
        
        _logger.LogInformation("OCR Response: {Response}", content);
        ...
        {
            _logger.LogError(ex, "Failed to parse OCR response: {Response}", cleanedJson);
            return new ImageOcrResult { Error = "Error al procesar la respuesta del análisis de imagen." };
        }
    }
